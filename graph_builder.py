...
class GraphBuilder:
	...
	def gen_giant_graph(self):
		...
		returns a giant graph
		"""
		
		for sent in self.sents[:self.limit]:
		...
		giant_graph = self.graphs[0]
		if len(self.graphs) > 1:
		
			for i in range(1, len(self.graphs)):
				giant_graph = nx.compose(self.graphs[i], giant_graph)
...
if __name__ == '__main__':
	...
	G.gen_data(sents)
	giant_graph = G.gen_giant_graph()
		# G.plot_graph(giant_graph)
	args.graph = giant_graph
	#print args.graph
	#args = Args(giant_graph, *args)
	args.meta_paths = G.meta_paths
	model = model_maker(args)
	#unique_dict = G.unique_dict
	visualize(model, './output/', giant_graph)
	#return model
